...
- Design-owning and implementation repository:
  `/Users/codex/Development/surgeist-generator`.
- Immutable implementation-series source base:
  `1e545530fb992e2e749f26a1ba7ac0d77ed85dd6`.
- Baseline findings:
  `plans/2026-07-17-crate-baseline-review.md` at commit
...
- Bounded outcome: rooted existence treats a missing intermediate component as
...
- Specification sections: SR-02 rows **Rooted lease-tree rejection** and
  ...
  **Quality matrix**; SR-04.1 through SR-04.4; the transaction, lease,
  bootstrap, owner-record, rename-probe, and shared error clauses of SR-04.5;
  and the current-core quality clauses of SR-04.6.
- Prerequisites: clean specification and sequence reviews; the source base and
  preserved-layout digest remain verified; no domain-driver implementation has
  begun.
- Entry state: the source base plus planning-only commits; rooted lease tests
...
- Owning repository: `surgeist-generator`.
- Bounded outcome: the protected-source/disjointness boundary and shared
  publication contracts drive one complete `css-corpus` feature and thin CSS
  binary with deterministic import, neutral expectation generation, checking,
  filtering, historical ownership, and atomic stale removal. Real CSS domain
...
- Entry state: shared recovery is cycle-clean, while `css-corpus` is inert and
  no CSS interface binary, import sidecar, expectation schema, or real domain
  linkage exists.
- Exit evidence: the exact CSS public API and binary execute all three commands
  against explicit synthetic roots; source protection, pinned import proof,
  neutral case ordering/dispositions, byte schemas, reports, full/filtered/check
  transitions, historical membership changes, reserved-path rejection, and real
  CLI error behavior satisfy their contract across the affected feature matrix.
- Handoff: publish and remotely verify the C02 leaf candidate, then provide its
...
The sequence allocates every contract exactly once at its final closure boundary:
