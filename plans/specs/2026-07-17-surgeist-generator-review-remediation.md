...
not execute those diagnostics under any feature combination. Exhaustive proof
executes the complete inventory explicitly and sequentially rather than through
an ordinary test matrix. Lightweight observer, corruption, error, and ordering
tests stay in the ordinary suite unless they themselves enumerate every real
prefix. No environment variable, machine state, or implicit timing rule selects
...
real-prefix diagnostics as ignored rather than execute them. Exhaustive
diagnostics are outside the ordinary matrix and use the explicit sequential proof
contract in SR-04.2.
